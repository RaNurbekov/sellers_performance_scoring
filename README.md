...
ML system that scores and segments 1,271 sellers into performance tiers 
using unsupervised learning — inspired by credit risk underwriting.
...
## Solution
Segmented sellers into 4 performance grades using KMeans clustering 
on behavioral metrics extracted from PostgreSQL via feature engineering.
...
## Key Insights
- **Grade C (Reliable)** sellers have the best review score (4.26) and 
  fastest delivery (9.6 days) despite low revenue — high growth potential
- **Grade D (At Risk)** sellers show 16.7 avg delivery days and 
  only 70.9% on-time rate — immediate intervention needed
- **SHAP analysis** reveals cancellation_rate and on_time_rate 
  are the strongest predictors of At Risk classification
